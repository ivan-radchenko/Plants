...
@section('content')
    ...
    <div class="container">
        <div class="sub-container">
            <img class="user-image" src="{{\Illuminate\Support\Facades\Storage::disk('public')->url($authUser->image)}}" alt="user_image" id="user-image">
            <form class="image-form" action="" method="post" enctype="multipart/form-data">
                    <div class="form-item">
                        ...
                        <input class="input-image" title="не обязательно" type="file" name="image" id="image" accept="image/png, image/jpeg, image/jpg">
                        @error('image')
        ...
        <div class="sub-container">
            <form class="name-form" action="" method="post">
                <div class="form-item">
                    ...
                    <input class="form-input" type="text" name="name" id="name" value="{{$authUser->name}}">
                    @error('name')
                    <span class="error">{{ $message }}</span>
                    @enderror
                    <button class="button" type="submit">Сохранить</button>
        ...
        <div class="sub-container">
            <form class="email-form" action="" method="post">
                <div class="form-item">
                    ...
                    <input class="form-input" type="email" name="email" id="email" value="{{$authUser->email}}">
                    @error('email')
                    <span class="error">{{ $message }}</span>
                    @enderror
                    <button class="button" type="submit">Сохранить</button>
    ...
    <script>
        ...
        document.getElementById('image').addEventListener('change', event => {
            document.getElementById('user-image').src=URL.createObjectURL(event.target.files[0]);
        })
    </script>
@endsection
